/*
 From the begining until line 1144 are results of using browserify to package external
 packages into this one file for external use. This allows us to use these packages
 inside an extension, which is normally impossible. 
*/
...
/*
This begins the code that we wrote for analyzing the article.
...
function get_row_with_name(first, last){
  /*
  Given a name, returns the row in the politicians dataset that has that name
  */
...
function is_name_in_string(paragraph_string, name){
  /*
  Tests if a name appears in a string, returning a boolean
  */
    if (name == ""){
        return false;
    }
...
function get_sentences(article_array){
  /*
  Divides the entire article into sentences for easier processing and viewing

  Input: an array of paragraphs of the article
...
/*
 What follows are modules that were imported into this file by the browserify tool.
 These are packages originally made for use in NodeJS, but repurposed by us for
